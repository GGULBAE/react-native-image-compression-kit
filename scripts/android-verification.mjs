...
function runDoctor() {
  const checks = [
    ...
    checkReleaseEvidenceReviewArchive(),
    checkWorkflowSupplyChain(),
    checkActionPinProvenance(),
...
function checkPackageMetadata() {
  ...
  const checks = [
    ...
    readmeContents.includes('aggregate archive SHA-256 `f63924d58ef18c94379b102949e6870e838a014ac883b7c9c03fca5abc6b56dd`'),
...
function checkReleaseEvidenceArchive() {
  ...
  const acquisitionCliContents = readText('scripts/acquire-release-evidence.mjs');
  const acquisitionRefreshContents = readText(
    'scripts/refresh-release-evidence-acquisition-fixtures.mjs'
  );
  const acquisitionTestContents = readText(
    'test/releaseEvidenceAcquisition.test.mjs'
  );
  const policyCoreContents = readText('scripts/release-evidence-policy-core.mjs');
  const policyPrepareContents = readText(
    'scripts/prepare-release-evidence-policy.mjs'
  );
  const policyPromotionContents = readText(
    'scripts/promote-release-evidence-policy.mjs'
  );
  ...
  const expectedSnippets = [
    ...
    [acquisitionCoreContents, 'validateReleaseEvidenceAcquisitionInputs'],
    [acquisitionCoreContents, 'selectReleaseEvidenceArtifacts'],
    [acquisitionCoreContents, 'ZIP digest does not match GitHub metadata'],
    [acquisitionCoreContents, 'Existing importer handoff failed:'],
    [acquisitionCoreContents, 'operations.rename(temporary, destination)'],
    [acquisitionGitHubContents, 'node:child_process'],
    [acquisitionGitHubContents, 'actions/artifacts/${artifactId}/zip'],
    [acquisitionGitHubContents, "run('unzip', ['-Z1', zipPath]"],
    [acquisitionCliContents, "'--run-id': 'runId'"],
    [acquisitionCliContents, "'--expected-tag': 'expectedTag'"],
    [acquisitionRefreshContents, 'Release evidence acquisition fixture'],
    [acquisitionTestContents, 'hands them to the existing importer byte-for-byte'],
    [acquisitionTestContents, 'requires every trust input explicitly and never selects the latest run'],
    [acquisitionTestContents, 'keeps validation core network-free and fixture checking offline'],
    [policyCoreContents, 'compareReleaseEvidencePolicy'],
    [policyCoreContents, 'Reviewed candidate SHA-256 does not match the candidate bytes.'],
    [policyCoreContents, 'Reviewed candidate is not the committed'],
    [policyCoreContents, 'Release evidence archive version already exists:'],
    [policyCoreContents, 'operations.rename(stagedArchive, destination)'],
    [policyPrepareContents, "'--candidate-file': 'candidateFile'"],
    [policyPromotionContents, "'--reviewed-candidate-sha256': 'reviewedCandidateSha256'"],
    [policyPromotionContents, "if (arg === '--approve')"],
    ...
    [readmeContents, 'pnpm acquire:release-evidence --'],
    [readmeContents, 'The command never selects the latest run.'],
    ...
    [readmeContents, 'Successful [Release Evidence Policy Review run 29390495773]'],
    [readmeContents, 'manifest SHA-256 `48cfd454b636cf1911b7d19dae996e7ead2797247d2b974687bb02aeebb439ff`'],
    [readmeContents, '[attestation 35388408]'],
    [readmeContents, 'receipt SHA-256 `45ddefa85cba6a9fed62cb1c187dd0bab2246b72ba66a803b1282e4eac07efad`'],
    ...
    [releaseContents, '### Explicit Acquisition Contract'],
    ...
    [releaseContents, '### Live v0.2.55 Review Receipt Evidence'],
    [releaseContents, 'Review artifact `8333046539` has GitHub digest `sha256:f1ea6c9c2498e4d773a6cc5f6b49d39d9bfacba8bd40ec76c5364c7d3c21c836`'],
    [releaseContents, 'attestation artifact `8333046693` has digest `sha256:05ab03d322d15e97cc733e3d0325f6dbb7a468197245ea9c6738241e2477f4d6`'],
    ...
    [securityContents, 'pnpm import:release-evidence-review --'],
    [securityContents, 'pnpm verify:release-evidence-review-archive -- --version 0.2.55 --json'],
    [securityContents, 'pnpm verify:release-evidence-review-archive-set -- --json'],
    [securityContents, 'intentionally does\nnot reject an already retained archive based on the current clock'],
  ...
  const scriptOk =
    ...
    packageJson.scripts?.['acquire:release-evidence'] ===
      'node scripts/acquire-release-evidence.mjs' &&
    packageJson.scripts?.['prepare:release-evidence-policy'] ===
    ...
    packageJson.scripts?.['fixtures:release-evidence-acquisition:check'] ===
      'node scripts/refresh-release-evidence-acquisition-fixtures.mjs --check' &&
  ...
  const acquisitionFixturesOk = [
    [
      'test/fixtures/release-evidence-acquisition/0.2.50/provenance.zip',
      70382,
      '031302873239c74234179041e6b3f7ff8d6fe281351dfb93a3f5e4ed9573ec71',
    ],
    [
      'test/fixtures/release-evidence-acquisition/0.2.50/attestation.zip',
      15537,
      '01a74cc10cb2c89e309d58ddf83f285ada1a3ceb3af70e78b8cd70dc6c627cea',
    ],
    [
      'test/fixtures/release-evidence-acquisition/0.2.55/provenance.zip',
      76792,
      '7463b03ff6294b5017d9b3cad05d4c3ea87b542398a5cb70f503cea148dca826',
    ],
    [
      'test/fixtures/release-evidence-acquisition/0.2.55/attestation.zip',
      15696,
      '545c63da880d9d91f9ade1cf40ce36a366634c11ff524b87579e6b0fd6d8e28f',
    ],
  ].every(([file, size, digest]) => {
  ...
  const ok =
    scriptOk &&
    indexOk &&
    acquisitionFixturesOk &&
    packageExcludesEvidence &&
    missing.length === 0;
  ...
  return {
    ...
    detail: ok
      ...
      : `release evidence contract mismatch: ${[
          ...
          ...(acquisitionFixturesOk ? [] : ['exact acquisition artifact ZIP fixtures']),
          ...(packageExcludesEvidence ? [] : ['package.json files includes evidence']),
        ].join(' | ')}`,
  };
}

function checkReleaseEvidenceReviewArchive() {
  const packageJson = readJson('package.json');
  const indexPath = 'evidence/reviews/0.2.55/review-evidence-index.json';
  const indexBytes = readFileSync(path.join(ROOT, indexPath));
  const index = JSON.parse(indexBytes.toString('utf8'));
  const zipCoreContents = readText('scripts/artifact-zip-core.mjs');
  const archiveCoreContents = readText(
    'scripts/release-evidence-review-archive-core.mjs'
  );
  const importCliContents = readText(
    'scripts/import-release-evidence-review-archive.mjs'
  );
  const verifyCliContents = readText(
    'scripts/verify-release-evidence-review-archive.mjs'
  );
  const setCoreContents = readText(
    'scripts/release-evidence-review-archive-set-core.mjs'
  );
  ...
  const setTestContents = readText(
    'test/releaseEvidenceReviewArchiveSet.test.mjs'
  );
  const readmeContents = readText('README.md');
  const releaseContents = readText('RELEASE.md');
  const securityContents = readText('SECURITY.md');
  const expectedSnippets = [
    [zipCoreContents, 'extractArtifactZip'],
    [zipCoreContents, 'Artifact ZIP path is unsafe'],
    [zipCoreContents, 'Artifact ZIP entry must be a regular file, not a directory or symlink'],
    [archiveCoreContents, 'RELEASE_EVIDENCE_REVIEW_ARCHIVE_INDEX_FIELDS'],
    [archiveCoreContents, 'RELEASE_EVIDENCE_REVIEW_ARCHIVE_POLICIES'],
    [archiveCoreContents, 'verifyReleaseEvidenceReviewBundle'],
    [archiveCoreContents, 'runReleaseEvidenceReviewAttestationVerification'],
    [archiveCoreContents, 'Imported review archive verification failed'],
    [importCliContents, "'--review-artifact-zip': 'reviewArtifactZip'"],
    [importCliContents, "'--attestation-artifact-zip': 'attestationArtifactZip'"],
    [verifyCliContents, 'writeReleaseEvidenceReviewArchiveVerificationAtomic'],
    [setCoreContents, 'DEFAULT_RELEASE_EVIDENCE_REVIEW_ARCHIVE_VERSIONS'],
    [setCliContents, 'writeReleaseEvidenceReviewArchiveSetReportAtomic'],
    [archiveTestContents, 'imports the exact ZIPs atomically with stdout and report bytes equal'],
    [archiveTestContents, "it.each(['traversal', 'duplicate', 'symlink', 'missing', 'additional'])"],
  ...
  const missing = expectedSnippets
    ...
    .map(([, snippet]) => snippet);
  const scriptsOk =
    packageJson.scripts?.['import:release-evidence-review'] ===
      'node scripts/import-release-evidence-review-archive.mjs' &&
    packageJson.scripts?.['verify:release-evidence-review-archive'] ===
  ...
  const indexOk =
    ...
    index.package === 'react-native-image-compression-kit' &&
    index.version === '0.2.55' &&
    index.candidateSha256 ===
      'aade4a8057bbb8f6b3dc92690b3d9cc5e3b57352a5734396e3921a143a449f8d' &&
    index.reviewer === 'GGULBAE' &&
    index.reviewRun?.id === 29390495773 &&
    index.reviewArtifact?.id === 8333046539 &&
    index.attestationArtifact?.id === 8333046693 &&
    index.attestation?.id === 35388408 &&
    index.receiptSha256 ===
  ...
  const zipFilesOk = [
    [
      'evidence/reviews/0.2.55/artifacts/review.zip',
      285466,
      'f1ea6c9c2498e4d773a6cc5f6b49d39d9bfacba8bd40ec76c5364c7d3c21c836',
    ],
    [
      'evidence/reviews/0.2.55/artifacts/attestation.zip',
      15751,
      '05ab03d322d15e97cc733e3d0325f6dbb7a468197245ea9c6738241e2477f4d6',
    ],
  ].every(([file, size, digest]) => {
    const bytes = readFileSync(path.join(ROOT, file));
    return bytes.length === size && sha256(bytes) === digest;
  ...
  return {
    ok,
    label: 'v0.2.55 review evidence is retained and replayed expiration-independently',
    detail: ok
      ? 'exact ZIPs, canonical index, complete extracted review evidence, atomic importer, offline single/set replay, documentation, default gate, and npm-package exclusion are present'
      : `review archive contract mismatch: ${[
          ...missing,
          ...(scriptsOk ? [] : ['package.json review archive scripts/default gate']),
          ...(indexOk ? [] : ['canonical review archive index identity']),
          ...(zipFilesOk ? [] : ['exact retained review artifact ZIPs']),
          ...(packageExcludesEvidence ? [] : ['package.json files includes evidence']),
        ].join(' | ')}`,
  };
}
...
function checkActionPinProvenance() {
  const packageJson = readJson('package.json');
  const coreContents = readText('scripts/action-pin-provenance-core.mjs');
  ...
  const fixtureManifestContents = readText(
    'test/fixtures/action-pin-review/artifact-manifest.json'
  );
  const fixtureManifest = JSON.parse(fixtureManifestContents);
  const fixtureEvent = readJson(
    'test/fixtures/action-pin-review/workflow-dispatch-event.json'
  );
  const fixtureExecution = readJson(
    'test/fixtures/action-pin-review/github-execution.json'
  );
  const fixtureWorkflow = readText(
    'test/fixtures/action-pin-review/action-pin-review-workflow.yml'
  );
  const rawEventFixture = readJson(
    'test/fixtures/github/action-pin-review-workflow-dispatch.json'
  );
  ...
  const securityContents = readText('SECURITY.md');
  const expectedSnippets = [
    [coreContents, 'ACTION_PIN_PROVENANCE_REPORT_FIELDS'],
    [coreContents, 'ACTION_PIN_PROVENANCE_EVIDENCE_FIELDS'],
    [coreContents, 'ACTION_PIN_ARTIFACT_MANIFEST_FIELDS'],
    [coreContents, 'createWorkflowDispatchEventEvidence'],
    [coreContents, 'createActionPinArtifactManifest'],
    [coreContents, 'validateManifestPath'],
    [coreContents, 'reviewActionPin'],
    [coreContents, 'verifyActionPinProvenanceArtifact'],
    [coreContents, 'writeActionPinProvenanceArtifactAtomic'],
    [coreContents, 'writeActionPinProvenanceReportAtomic'],
    [resolverContents, 'resolveGitHubActionTag'],
    [resolverContents, 'await fetch(url'],
    [reviewCliContents, "'--baseline-lock': 'baselineLock'"],
    [reviewCliContents, "'--candidate-lock': 'candidateLock'"],
    [reviewCliContents, "'--source-head-sha': 'sourceHeadSha'"],
    [reviewCliContents, "'--workflow-ref': 'workflowRef'"],
    [reviewCliContents, "'--github-event': 'githubEvent'"],
    ...
    [testContents, 'rejects execution, event, and workflow identity mismatches'],
    [testContents, 'rejects annotated-tag dereference failure'],
    [testContents, 'rejects artifact manifest path traversal'],
    [testContents, 'rejects missing and additional artifact files'],
    [testContents, 'rejects artifact evidence size and SHA-256 tampering'],
    [testContents, 'rejects provenance report execution identity tampering'],
    [testContents, 'atomically preserves prior reports and removes incomplete artifact directories'],
    [workflowContents, 'workflow_dispatch:'],
    [workflowContents, 'baseline_ref:'],
    [workflowContents, 'name: Verify candidate workflow lock'],
    [workflowContents, 'pnpm review:action-pin --'],
    [workflowContents, 'pnpm verify:action-pin-provenance --'],
    [workflowContents, 'SOURCE_HEAD_SHA: ${{ github.sha }}'],
    ...
    [readmeContents, '### Manual Action pin provenance review'],
    [readmeContents, '`artifact-manifest.json`'],
    ...
    [securityContents, 'source repository/ref/head SHA'],
  ...
  const missing = expectedSnippets
    ...
    .map(([, snippet]) => snippet);
  const scriptsOk =
    packageJson.scripts?.['review:action-pin'] ===
  ...
  const fixtureOk =
    ...
    fixtureReport.action === 'gradle/actions/setup-gradle' &&
    fixtureReport.releaseTag === 'v6' &&
    fixtureReport.resolution === 'annotated' &&
    fixtureReport.resolvedCommitSha ===
      '3f131e8634966bd73d06cc69884922b02e6faf92' &&
    fixtureReport.sourceRepository ===
    ...
    fixtureReport.evidence?.executionSha256 ===
      sha256(Buffer.from(`${JSON.stringify(fixtureExecution)}\n`, 'utf8')) &&
    fixtureManifest.schemaVersion === 1 &&
    fixtureManifest.status === 'passed' &&
    JSON.stringify(fixtureManifest.files?.map((entry) => entry.path)) ===
      JSON.stringify([
    ...
    fixtureEvent.eventName === 'workflow_dispatch' &&
    fixtureExecution.sourceRepository === fixtureReport.sourceRepository &&
    fixtureExecution.sourceHeadSha === fixtureReport.sourceHeadSha &&
    fixtureExecution.workflowRef === fixtureReport.workflowRef &&
    fixtureExecution.runId === fixtureReport.runId &&
    fixtureExecution.runAttempt === fixtureReport.runAttempt &&
    fixtureEvent.repository === 'GGULBAE/react-native-image-compression-kit' &&
    fixtureEvent.workflow === '.github/workflows/action-pin-review.yml' &&
  ...
  const packageExcludesReviewFiles = !(
    packageJson.files ?? []
  ).some((entry) => ['.github', 'scripts', 'test', 'evidence'].includes(entry));
  const ok =
    scriptsOk &&
  ...
  return {
    ok,
    label: 'Action pin provenance binds GitHub execution identity and a canonical artifact manifest',
    detail: ok
      ? 'network tag resolution, source/workflow/run identity, workflow and lock hashes, canonical manifest, annotated event fixture, manual summary/upload gate, offline replay, and npm exclusions are aligned'
      : `Action pin provenance contract mismatch: ${[
          ...missing,
...
function checkReleaseNotes() {
  ...
  const releaseSnippets = [
    ...
    '`pnpm import:release-evidence-review -- --version <version> --metadata-file <metadata.json> --review-artifact-zip <review.zip> --attestation-artifact-zip <attestation.zip> --archive-dir <archive> --release-archive-root evidence/npm --json --report-file <report.json>`',
    'The ordered index fields are `schemaVersion`, `status`, `package`, `version`, `candidateSha256`, `reviewer`, `reviewedAt`, `repository`, `workflow`, `sourceRef`, `sourceDigest`, `reviewRun`, `reviewArtifact`, `attestationArtifact`, `attestation`, `receiptSha256`, `manifestSha256`, `evidenceSha256`, `files`, `archiveSha256`, and `error`.',
    '`pnpm verify:release-evidence-review-archive-set -- --json --report-file <report.json>`',
    '### Retained v0.2.55 Review Evidence',
    'Canonical aggregate archive SHA-256 is `f63924d58ef18c94379b102949e6870e838a014ac883b7c9c03fca5abc6b56dd`',
    '## v0.2.59',
    ...
    '`pnpm prepare:release-evidence-policy -- --acquisition-dir <path> --candidate-file <candidate.json> --json --report-file <report.json>`',
    '### Reviewed Promotion Gate',
    '`pnpm promote:release-evidence-policy -- --acquisition-dir <path> --candidate-file <candidate.json> --version <version> --reviewed-candidate-sha256 <sha256> --reviewer <identity> --reviewed-at <UTC-ISO> --archive-root <path> --approve --json --report-file <report.json>`',
    'Preparation cannot edit policy source and promotion has no `--apply` option.',
    '### Live v0.2.55 Replay Evidence',
    'candidate bytes at SHA-256 `aade4a8057bbb8f6b3dc92690b3d9cc5e3b57352a5734396e3921a143a449f8d`',
    '## v0.2.57',
    ...
    '`pnpm acquire:release-evidence -- --repository <owner/repo> --workflow <.github/workflows/file.yml> --source-ref <refs/heads/name> --source-digest <40-char-sha> --run-id <id> --version <version> --expected-tag <tag> --output-dir <path> --json --report-file <report.json>`',
    '### Canonical Handoff Contract',
    'The ordered acquisition report fields are `schemaVersion`, `status`, `outputDir`, `package`, `version`, `expectedTag`, `repository`, `runId`, `sourceDigest`, `acquisitionSha256`, `evidenceSha256`, `checks`, and `error`.',
    'Exact GitHub artifact ZIPs for v0.2.50 and v0.2.55 are retained below `test/fixtures/release-evidence-acquisition/`.',
    'canonical handoff acquisition SHA-256 was `1545317c2047808f35f253a1387f7a019b2174ca317cbcb6b325b6ac1b797681`',
    '## v0.2.56',
    ...
    '`pnpm review:action-pin` command keeps the v0.2.53 tag resolver boundary and additionally accepts `--baseline-ref`, `--source-repository`, `--source-ref`, `--source-head-sha`, `--workflow-name`, `--workflow-path`, `--workflow-ref`, `--workflow-sha`, `--workflow-file`, `--run-id`, `--run-attempt`, `--event-name`, and `--github-event`',
    '`pnpm verify:action-pin-provenance -- --artifact-dir <path> --json` validates canonical report/manifest bytes',
    'The schema v2 report adds ordered `sourceRepository`, `sourceRef`, `sourceHeadSha`, `workflowName`, `workflowPath`, `workflowRef`, `workflowSha`, `runId`, and `runAttempt` fields.',
    ...
    '`pnpm review:action-pin` command accepts `--action`, `--repository`, `--release-tag`, `--proposed-sha`, `--baseline-lock`, `--candidate-lock`, and `--artifact-dir`',
    '`pnpm verify:action-pin-provenance -- --artifact-dir <path> --json`',
    'The five files under `.github/workflows/` now contain 28 full-SHA remote Action uses for the same nine unique Actions.',
    'The updated canonical lock SHA-256 is `75bfabd61eb14ad5f26320916ae642c603ca509e942a1c962d97e08c750c6777`.',
    '## v0.2.52',
...
function checkSecurityPolicy() {
  ...
  const expectedSnippets = [
    ...
    [securityContents, 'Before accepting a proposed Action SHA change'],
    [securityContents, 'pnpm verify:action-pin-provenance --'],
    [securityContents, 'unregistered Actions, repository substitution, major-version'],
    [securityContents, 'must not enter the npm package'],
