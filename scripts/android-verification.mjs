...
function checkPackageMetadata() {
  ...
  const checks = [
    ...
    readmeContents.includes('Version `0.2.18` is an unpublished docs-only npm README correction candidate for `react-native-image-compression-kit`'),
    readmeContents.includes('It keeps Android and iOS runtime behavior unchanged while preparing a package-page README that no longer carries stale `0.2.17` pre-publish candidate wording.'),
    readmeContents.includes('The latest published npm package is `0.2.17`'),
    readmeContents.includes('GitHub Release [v0.2.17]'),
    readmeContents.includes('The `0.2.18` package metadata is prepared as an unpublished docs-only npm README correction candidate for `react-native-image-compression-kit`'),
    ...
    readmeContents.includes('version `0.2.18` is the unpublished docs-only npm package-page README correction candidate'),
    ...
    readmeContents.includes('Version `0.2.18` corrects the prepared npm README without runtime behavior changes.'),
  ...
  return {
    ...
    label: 'npm package metadata and README status are aligned for the v0.2.18 docs-only npm README correction candidate',
    detail: checks.every(Boolean)
      ? 'name, version, license, repository, bugs, homepage, exports, peer dependency, keywords, and README docs-only candidate status are aligned'
...
function checkReleaseNotes() {
  ...
  const releaseSnippets = [
    ...
    'This candidate corrects the README that is shown on the npm package page after the `0.2.17` tarball shipped pre-publish candidate status text.',
  ...
  const ok = packageJson.version === '0.2.18' && missing.length === 0;

  return {
    ok,
    label: 'v0.2.18 docs-only npm README correction notes and previous release notes are current',
    detail: ok
