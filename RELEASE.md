...
This candidate keeps native behavior, the public API, npm latest, committed release evidence policies, and `evidence/npm` unchanged. It turns a reviewed policy-candidate digest into durable GitHub execution evidence and proves the exact acquisition-to-promotion path in an isolated, complete archive set before emitting one self-contained artifact.

### Goals

- Require an explicit Registry Validation repository/workflow/ref/source SHA/run ID/version/tag and reviewed candidate SHA-256; never select a latest run.
- Treat the manual dispatch actor as the reviewer and bind that actor to the exact normalized dispatch inputs, workflow repository/path/ref/SHA, review source commit, run ID/attempt, and GitHub run start time.
- Reuse acquisition, candidate preparation, reviewed promotion, and complete release evidence set verification without changing their trust boundaries.
- Copy only non-target retained archives into an isolated rehearsal set, promote the target there, verify every committed policy version, and leave the repository archive untouched.
- Atomically emit acquisition, candidate, diff, execution, event, workflow, promotion, set, receipt, complete archive set, and recursive manifest evidence only after a second offline replay succeeds.
- Attest the canonical recursive manifest, retain the downloaded Sigstore bundle and pinned trusted root, and verify signer workflow/source/invocation offline.
- Keep default `pnpm verify`, bundle construction/replay, and downloaded-attestation verification network-free.

### Canonical Review Receipt

`pnpm review:release-evidence-policy -- --acquisition-dir <path> --candidate-file <candidate.json> --policy-report-file <diff.json> --archive-root evidence/npm --bundle-dir <artifact> --reviewed-candidate-sha256 <sha256> <explicit-review-and-registry-identity> --json --report-file <receipt.json>` is the offline rehearsal entrypoint used after explicit acquisition and candidate preparation.

The ordered receipt fields are `schemaVersion`, `status`, `package`, `version`, `candidateSha256`, `reviewer`, `reviewedAt`, `repository`, `workflow`, `workflowSha`, `reviewRunId`, `reviewRunAttempt`, `sourceRef`, `sourceDigest`, `acquisitionSha256`, `evidenceSha256`, `promotionReportSha256`, `setReportSha256`, and `error`.

The review identity is valid only when the workflow is exactly `.github/workflows/release-evidence-policy-review.yml`, workflow SHA equals the review source commit, the workflow ref binds repository/path/ref, run ID and attempt are positive, the dispatch event repository/ref/workflow/actor agree, and all eight explicit dispatch inputs equal the candidate policy and reviewed digest. The review timestamp is resolved from the GitHub run and may not precede Registry Validation completion.

### Standalone Promotion Rehearsal Bundle

The artifact contains `acquisition/`, `archive-set/`, `policy-candidate.json`, `policy-diff.json`, `github-execution.json`, `workflow-dispatch-event.json`, `release-evidence-policy-review-workflow.yml`, `promotion-report.json`, `release-evidence-set-report.json`, `review-receipt.json`, and `artifact-manifest.json`. The archive set includes every committed policy version; for historical v0.2.55 rehearsal, v0.2.50 is copied as the baseline and v0.2.55 is regenerated by reviewed promotion.

Every regular file below acquisition and archive-set is listed recursively with ordered `path`, `size`, and `sha256`; symlinks, unsafe paths, missing/additional files, and manifest drift fail closed. The manifest does not describe itself and is the only attestation subject.

Before the output directory is exposed, the builder verifies the copied baselines, promotes into staging, verifies the complete set, writes portable relative-path reports, recreates the candidate from acquisition, repeats promotion into a new temporary set, compares the reproduced reports, and byte-compares the target archive. Candidate/acquisition disagreement, reviewed digest mismatch, policy drift, duplicate target, promotion/set failure, bundle verification failure, or bundle/report rename failure removes staging and leaves no success receipt or artifact.

`pnpm verify:release-evidence-review -- --artifact-dir <artifact> --expect-package <name> --expect-version <version> --expect-candidate-sha256 <sha256> --expect-reviewer <actor> --expect-repository <owner/repo> --expect-workflow <owner/repo/.github/workflows/release-evidence-policy-review.yml> --expect-ref <refs/...> --expect-head-sha <sha> --expect-run-id <id> --expect-run-attempt <n> --json --report-file <report.json>` performs the same replay without network access and returns the bundled receipt byte-for-byte.

### Manual Workflow and Attestation

The `Release Evidence Policy Review` workflow has fixed explicit inputs `repository`, `workflow`, `source_ref`, `source_digest`, `registry_validation_run_id`, `version`, `expected_tag`, and `reviewed_candidate_sha256`. It acquires the immutable Registry Validation artifacts, prepares the candidate/diff, builds and replays the review bundle, compares builder/verifier stdout and report bytes, attests `artifact-manifest.json`, downloads the attestation and trusted root, verifies both online and offline, records receipt/manifest evidence in the GitHub Step Summary, and uploads separate review and attestation artifacts.
...
- Canonical execution/event/receipt/manifest schemas and a recursively complete, atomic review artifact.
- Temporary full archive-set promotion rehearsal using the existing candidate, promotion, importer, and set-verifier contracts.
- Manual review workflow with immutable Action pins, GitHub OIDC manifest attestation, offline downloaded-bundle verification, Step Summary, and artifact upload.
- Offline tests for canonical success, reviewer/run/workflow/ref/source identity drift, candidate/acquisition or digest mismatch, policy drift, duplicate target, event/manifest tampering, promotion/set failure, and bundle/report atomic failures.
- README, release notes, security guidance, workflow lock, Android doctor, and Vitest expectations aligned to v0.2.59 candidate status and npm latest v0.2.55.

### Not Included

- npm publish, dist-tag changes, git tags, GitHub Releases, native/API changes, or AVIF output implementation.
- Policy source edits, actual `evidence/npm` mutation, automatic commit/push/PR creation, or unattended candidate selection/approval.
- GitHub/npm credentials, OTPs, `.npmrc`, or tokens in repository files, logs, receipts, manifests, or uploaded artifacts.
- Registry/GitHub access from default verification or any offline review verifier.

### Validation

- Blocked-network `pnpm verify`
- `pnpm example:typecheck`
- `git diff --check`
- `pnpm pack --dry-run`
- `pnpm release:dry-run`
...
- Review bundle verifier output and report byte equality, target archive byte equality with retained v0.2.55, and repository archive non-mutation.
- Manual Release Evidence Policy Review workflow artifact, GitHub Step Summary, and manifest attestation verification.
- GitHub Actions CI, Android Instrumentation, and iOS Validation green on the pushed candidate commit.
...
- README, release notes, Android verification doctor checks, and Vitest expectations aligned to the registry-independent v0.2.50 tarball wording and final npm latest v0.2.50 evidence.

### Not Included

- Additional npm publish attempts, authentication storage, or manual dist-tag changes outside the completed single `latest` promotion.
...
- Registry Validation dispatched with `version=0.2.50` and `expected_tag=latest`.
...
- GitHub Actions CI, Android Instrumentation, and iOS Validation on the pushed release-ready and post-publish evidence commits.

### Promotion Result

- Release-ready commit `2b198c5f6125de6ad5bae76fc835ff5b935984f0` passed GitHub Actions [CI](https://github.com/GGULBAE/react-native-image-compression-kit/actions/runs/29309102114), [Android Instrumentation](https://github.com/GGULBAE/react-native-image-compression-kit/actions/runs/29309102109), and [iOS Validation](https://github.com/GGULBAE/react-native-image-compression-kit/actions/runs/29309102117).
...
- Registry metadata reports `version=0.2.50`, `dist-tags.latest=0.2.50`, publish time `2026-07-14T06:05:27.963Z`, tarball `https://registry.npmjs.org/react-native-image-compression-kit/-/react-native-image-compression-kit-0.2.50.tgz`, integrity `sha512-FCIEFSpcbb3pUbk7jVWuQxke0+h77q3f2jAtK2C3GILlvi6X7MvjSLH0BBMqd94qHdeluFclW6Rft+SjpjK/rw==`, and shasum `da8779a4fb67e52ef081302eec76aa12706691a2`.
- `pnpm smoke:registry -- --version 0.2.50 --expect-tag latest --json --artifact-dir registry-validation` matched 51 files, 68,615-byte package size, and 298,905-byte unpacked size; README status, required/forbidden contents, integrity/shasum, and clean consumer install/typecheck passed. The captured stdout, `stdout.json`, and `registry-provenance.json` were byte-identical at SHA-256 `1c6d7e6478fcb910653e8f607fb2bc5babdccc4a0c2e361d2c5a4a5d173d68b3`.
- Manual [Registry Validation run 29310375801](https://github.com/GGULBAE/react-native-image-compression-kit/actions/runs/29310375801) passed for `version=0.2.50` and `expected_tag=latest`; registry validation, manifest attestation, online and downloaded-bundle offline attestation verification, Step Summary generation, and both artifact uploads completed successfully.
- [Attestation 35201998](https://github.com/GGULBAE/react-native-image-compression-kit/attestations/35201998) binds `bundle-manifest.json` SHA-256 `0a152ce1989267ce5c2fa01096a5e0dc44200245e29843857b24c52d0b746773` to repository `GGULBAE/react-native-image-compression-kit`, signer workflow `GGULBAE/react-native-image-compression-kit/.github/workflows/registry-validation.yml`, ref `refs/heads/master`, source digest `2b198c5f6125de6ad5bae76fc835ff5b935984f0`, GitHub Actions OIDC, SLSA v1, and Tlog timestamp `2026-07-14T06:07:06.000Z`.
- The exact four-file `registry-provenance-0.2.50` artifact has GitHub digest `sha256:031302873239c74234179041e6b3f7ff8d6fe281351dfb93a3f5e4ed9573ec71`; the separate three-file `registry-provenance-attestation-0.2.50` artifact has GitHub digest `sha256:01a74cc10cb2c89e309d58ddf83f285ada1a3ceb3af70e78b8cd70dc6c627cea`.
- Downloaded provenance SHA-256 values are `0a152ce1989267ce5c2fa01096a5e0dc44200245e29843857b24c52d0b746773` for the manifest, `e737b19ba66a129abefc717186c6cb559fabb01db863f9ca76b90ece487e5033` for the tarball, and `1c6d7e6478fcb910653e8f607fb2bc5babdccc4a0c2e361d2c5a4a5d173d68b3` for both report and stdout. Attestation artifact SHA-256 values are `59eb3fd15c7392db7c93fdb69963a45778d2de54851bdc7bda951945e201d6a4` for `attestation.jsonl`, the pinned `65ca537f6ed8a47fd0e560c421baa1f6c1efb8b25fc200d8c5c02c0e92eb2b9c` for `trusted-root.jsonl`, and `380574a9b985e7d046953fa1338d47437753097ee531af85990d0257b3addb8e` for `attestation-verification.json`.
...
- No additional publish attempt, manual dist-tag change, git tag, GitHub Release, npm token, OTP, `.npmrc`, or authentication file was stored or created in the repository.
