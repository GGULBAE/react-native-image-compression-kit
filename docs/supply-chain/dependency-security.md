...
adopted by Lighthouse's upstream prerelease line:
...
and Lighthouse 13.4.0 to Sentry 10.66.0 to `@opentelemetry/core` 2.9.0. Vitest
keeps its independent Vite 8.0.16 resolution. This avoids adopting a
VitePress/Lighthouse prerelease or changing package runtime dependencies,
public APIs, native code, React Native support, or codec behavior.
...
pnpm why vite esbuild @opentelemetry/core
pnpm audit --json
pnpm site:check
pnpm site:build
pnpm site:quality
...
resolution against the minimum safe versions, presence of the reviewed Vite
resolution, and absence of these tools from production dependency fields.
Mutation tests prove that a missing override, vulnerable resolution, or
production exposure fails. `pnpm audit` is the separate networked registry
cross-check and must report zero advisories before merge.
