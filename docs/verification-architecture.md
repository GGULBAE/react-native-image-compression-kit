...
| Native demo result metrics, source/output/screenshot/recording bytes, ordered walkthrough timing, parsed MP4 duration, digests, platform provenance, and exact source identity | `test/demoEvidence.test.mjs`, `test/guidedDemoCore.test.mjs`, `scripts/demo-evidence-core.mjs`, and `scripts/guided-demo-core.mjs` | Native Demo Evidence workflow and `pnpm verify:demo-evidence` |
