...
| iOS bridge, immutable request/source/inspection/decoder/transform/JPEG-metadata boundaries, pod, workflow, and native-test/smoke-runner wiring | `test/iosSourceContract.test.ts` | `pnpm test` and `pnpm android:doctor` |
...
the iOS request/input/decoder/transformer/JPEG-metadata native tests plus
host-app smoke test in their supported environments.
