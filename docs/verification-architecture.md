...
| iOS bridge, immutable request/source/inspection/decoder/transform boundaries, pod, workflow, and native-test/smoke-runner wiring | `test/iosSourceContract.test.ts` | `pnpm test` and `pnpm android:doctor` |
...
the iOS request/input/decoder/transformer native tests plus host-app smoke test
