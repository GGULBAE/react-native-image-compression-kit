...
<p align="center">
  <img alt="Status: v0.2.45 candidate" src="https://img.shields.io/badge/Status-v0.2.45%20candidate-yellow" />
...
Version `0.2.45` is an unpublished iOS PASS replay fixture offline refresh artifact candidate for `react-native-image-compression-kit`. It keeps Android and iOS runtime behavior unchanged, keeps AVIF output disabled, does not force AVIF input availability or unavailability, does not force WebP output availability, and moves the replay provenance plus exact `RNICK_IOS_SMOKE_PASS` source line into `test/fixtures/ios-smoke-pass-ci-replay.json`. `scripts/ios-smoke-pass-replay-fixture.mjs` validates the schema version, exact root/provenance fields, GitHub Actions prefix metadata, PASS JSON, run URL, head SHA, and source-line SHA-256. `scripts/refresh-ios-smoke-pass-replay.mjs` reads a downloaded local Actions log plus provenance arguments, extracts exactly one PASS line, derives job/step/timestamp fields, calculates SHA-256, and writes deterministic canonical JSON without network access. `test/iosSmokePassReplayFixture.test.mjs` pins the schema and byte output with fake logs, runs the CLI twice for deterministic equality, and rejects missing, duplicate, malformed, and hash-mismatched source lines. The existing matrix fixture factory still covers all four WebP output x AVIF input combinations, including platform, JPEG/PNG/GIF/WebP/HEIC/HEIF result byte fields, conditional AVIF result byte fields, PNG-output result byte fields, conditional WebP output byte fields, `webpTargetSizeResultBytes`, `targetSizeResultBytes`, `webpOutputAvailable`, `avifInputAvailable`, `unsupportedInputs`, and `unsupportedOutputs`. The workflow still tees failed smoke output to `ios-smoke-diagnostics/ios-smoke.log`, generates `ios-smoke-diagnostics/ios-smoke-summary.md`, and uploads the `ios-smoke-diagnostics` artifact only through `if: failure()` steps. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.40`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).
...
Version `0.2.40` remains the latest published npm package and iOS AVIF-input unavailable PASS payload schema snapshot release. No npm publish, git tag, or GitHub Release is part of the v0.2.45 candidate.
...
Version `0.2.43` is the unpublished iOS PASS payload replay fixture provenance candidate. Version `0.2.44` is the unpublished iOS PASS replay fixture source-line integrity digest candidate. Version `0.2.45` is the unpublished iOS PASS replay fixture offline refresh artifact candidate.
...
Version `0.2.43` hardens iOS PASS payload replay fixture provenance and refresh guidance without enabling AVIF output. Version `0.2.44` hardens iOS PASS replay fixture source-line SHA-256 integrity without enabling AVIF output. Version `0.2.45` adds the structured replay artifact and offline deterministic refresh CLI without enabling AVIF output.
