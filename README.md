...
<p align="center">
  <img alt="Status: v0.2.54 candidate" src="https://img.shields.io/badge/Status-v0.2.54%20candidate-blue" />
...
Version `0.2.54` is the unpublished Action pin provenance execution identity and artifact manifest binding candidate. npm `latest` remains `0.2.50`; no npm publish, dist-tag change, git tag, or GitHub Release is part of this candidate. It keeps native behavior, the public API, and retained v0.2.50 evidence unchanged while binding every manual Action Pin Review report to its source repository/ref/head SHA, workflow identity, run ID/attempt, exact workflow definition, canonical Action lock, normalized dispatch event, and a complete evidence manifest that replays offline.
