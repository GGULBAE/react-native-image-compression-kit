...
<p align="center">
  <img alt="Status: v0.2.52 candidate" src="https://img.shields.io/badge/Status-v0.2.52%20candidate-blue" />
...
Version `0.2.52` is the unpublished immutable GitHub Actions pin and workflow supply-chain gate candidate. npm `latest` remains `0.2.50`; no npm publish, dist-tag change, git tag, or GitHub Release is part of this candidate. It keeps native behavior, the public API, and retained v0.2.50 evidence unchanged while pinning every remote workflow Action to a full commit SHA and checking the workflows against a canonical repository-owned lock without network access.
...
The repository package metadata is `0.2.52` for the unpublished immutable GitHub Actions pin and workflow supply-chain gate candidate. npm `latest` remains `0.2.50`. Repository, issue tracker, homepage, MIT license, React Native peer dependency, CommonJS entrypoint, TypeScript declarations, React Native Codegen source, Android main sources, and iOS native source remain included in a package tarball; `.github/`, repository scripts/tests, and the repository-owned `evidence/` archive remain excluded.
...
`pnpm verify` runs type checking, unit tests, the TypeScript build, `pnpm fixtures:ios-pass-replay:audit`, the committed `pnpm verify:release-evidence -- --version 0.2.50` replay, `pnpm verify:workflow-supply-chain -- --json`, and the Android verification doctor. Both committed supply-chain gates are network-free.
