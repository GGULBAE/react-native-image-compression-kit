...
<p align="center">
  <img alt="Status: v0.2.46 candidate" src="https://img.shields.io/badge/Status-v0.2.46%20candidate-yellow" />
...
Version `0.2.46` is an unpublished iOS PASS replay fixture offline check mode candidate for `react-native-image-compression-kit`. It keeps Android and iOS runtime behavior unchanged, keeps AVIF output disabled, does not force AVIF input availability or unavailability, does not force WebP output availability, and adds a read-only freshness check for `test/fixtures/ios-smoke-pass-ci-replay.json`. `scripts/ios-smoke-pass-replay-fixture.mjs` validates the schema and reports deterministic `schemaVersion`, `provenance.<field>`, and `sourceLine` differences. `scripts/refresh-ios-smoke-pass-replay.mjs` still refreshes deterministic canonical JSON from a downloaded local Actions log and provenance arguments, while `--check` builds the expected fixture in memory, validates the existing artifact, compares exact canonical bytes, reports `canonicalFormat` when only formatting differs, and exits without writing. `test/iosSmokePassReplayFixture.test.mjs` covers current, stale provenance, stale source-line/digest, missing, malformed, invalid-schema, and noncanonical artifacts with fake logs while pinning unchanged file bytes and modification times. The existing matrix fixture factory still covers all four WebP output x AVIF input combinations, including platform, JPEG/PNG/GIF/WebP/HEIC/HEIF result byte fields, conditional AVIF result byte fields, PNG-output result byte fields, conditional WebP output byte fields, `webpTargetSizeResultBytes`, `targetSizeResultBytes`, `webpOutputAvailable`, `avifInputAvailable`, `unsupportedInputs`, and `unsupportedOutputs`. The workflow still tees failed smoke output to `ios-smoke-diagnostics/ios-smoke.log`, generates `ios-smoke-diagnostics/ios-smoke-summary.md`, and uploads the `ios-smoke-diagnostics` artifact only through `if: failure()` steps. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.40`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).
...
The check exits `0` when the existing artifact is valid, canonical, and current. Missing, malformed, invalid, stale, or noncanonical artifacts exit `1` with concise schema, provenance, digest, source-line, or canonical-format differences. Check mode performs no writes and neither mode performs network requests.

Version `0.2.40` remains the latest published npm package and iOS AVIF-input unavailable PASS payload schema snapshot release. No npm publish, git tag, or GitHub Release is part of the v0.2.46 candidate.
...
The CI replay fixture artifact coverage validates `test/fixtures/ios-smoke-pass-ci-replay.json`, its canonical JSON formatting, workflow, run, head SHA, job, step, timestamp, source URL, exact PASS source line, and SHA-256. Fake-log CLI coverage proves the local refresh path is deterministic and network-free.
...
The current v0.2.46 iOS PASS replay fixture offline check mode candidate notes are in [RELEASE.md](RELEASE.md).
