...
<p align="center">
  <img alt="Status: v0.2.53 candidate" src="https://img.shields.io/badge/Status-v0.2.53%20candidate-blue" />
...
Version `0.2.53` is the unpublished GitHub Action pin update provenance and manual review gate candidate. npm `latest` remains `0.2.50`; no npm publish, dist-tag change, git tag, or GitHub Release is part of this candidate. It keeps native behavior, the public API, and retained v0.2.50 evidence unchanged while resolving reviewed lightweight or annotated Action tags in a manual workflow, binding the result to baseline and candidate canonical locks, and retaining an offline-replayable provenance artifact.
...
The validator requires the Action in both locks, an unchanged owner/repository, no major-version downgrade, exact candidate release-tag and SHA agreement, exact GitHub API repository/tag/object URLs, a valid lightweight or single annotated-tag dereference, and final tag-to-proposed-commit equality. It never edits workflow pins or the lock.

The canonical artifact contains baseline and candidate lock bytes, `tag-reference.json`, optional `annotated-tag.json`, and `action-pin-provenance.json`. The report fields are ordered as `schemaVersion`, `status`, `action`, `repository`, `releaseTag`, `proposedSha`, `baselineVersion`, `baselineSha`, `candidateVersion`, `candidateSha`, `tagObjectType`, `tagObjectSha`, `resolvedCommitSha`, `resolution`, `evidence`, `checks`, and `error`. Evidence pins every bundled filename and SHA-256; ordered checks are `inputs`, `registration`, `repository`, `releaseTag`, `noDowngrade`, `candidateLock`, `tagReference`, `dereference`, and `commit`. Artifact creation and standalone `--report-file` writes use temporary siblings and atomic rename so partial reports are never exposed.
...
The committed annotated-tag fixture for `gradle/actions/setup-gradle@v6` exercises this replay from default `pnpm verify`. Only `scripts/action-pin-review-github.mjs` performs GitHub requests; the validation core, offline verifier, fixture gate, and default verify path contain no network or command-execution route. The manual workflow writes a GitHub Step Summary and uploads the exact artifact, but it never changes a pin, lock, Dependabot PR, npm package, or trusted root.
...
The repository package metadata is `0.2.53` for the unpublished GitHub Action pin update provenance and manual review gate candidate. npm `latest` remains `0.2.50`. Repository, issue tracker, homepage, MIT license, React Native peer dependency, CommonJS entrypoint, TypeScript declarations, React Native Codegen source, Android main sources, and iOS native source remain included in a package tarball; `.github/`, repository scripts/tests/fixtures, generated Action pin review artifacts, and the repository-owned `evidence/` archive remain excluded.
...
`pnpm verify` runs type checking, unit tests, the TypeScript build, `pnpm fixtures:ios-pass-replay:audit`, the committed `pnpm verify:release-evidence -- --version 0.2.50` replay, `pnpm verify:workflow-supply-chain -- --json`, the committed `pnpm verify:action-pin-fixture`, and the Android verification doctor. All committed replay and supply-chain gates are network-free; only the separately invoked `pnpm review:action-pin` command resolves GitHub tags.
...
Use the manual **Action Pin Review** workflow or `pnpm review:action-pin` only when reviewing a proposed SHA update. Downloaded artifacts can always be replayed with `pnpm verify:action-pin-provenance -- --artifact-dir <path> --json`.
