...
<p align="center">
  <img alt="Status: v0.2.51 candidate" src="https://img.shields.io/badge/Status-v0.2.51%20candidate-blue" />
...
Version `0.2.51` is the unpublished expiration-independent release evidence archive and offline replay gate candidate. npm `latest` remains `0.2.50`; no npm publish, dist-tag change, git tag, or GitHub Release is part of this candidate. It keeps native behavior, the public API, and the registry provenance/attestation schemas unchanged while moving the validated v0.2.50 evidence under repository ownership.
...
The repository package metadata is `0.2.51` for the unpublished expiration-independent release evidence archive and offline replay gate candidate. npm `latest` remains `0.2.50`. Repository, issue tracker, homepage, MIT license, React Native peer dependency, CommonJS entrypoint, TypeScript declarations, React Native Codegen source, Android main sources, and iOS native source remain included in a package tarball; the repository-owned `evidence/` archive remains excluded.
...
`pnpm verify` runs type checking, unit tests, the TypeScript build, `pnpm fixtures:ios-pass-replay:audit`, the committed `pnpm verify:release-evidence -- --version 0.2.50` offline replay, and the Android verification doctor.
