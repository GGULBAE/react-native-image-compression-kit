...
<p align="center">
  <img alt="Status: v0.2.22 candidate" src="https://img.shields.io/badge/Status-v0.2.22%20candidate-yellow" />
...
Version `0.2.22` is an unpublished Android AVIF output production helper extraction candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while extracting the Android AVIF encode/decode-back path into `AndroidAvifOutputHelper` with reusable helper input, output, sample, validation, and result types. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.22 candidate.
