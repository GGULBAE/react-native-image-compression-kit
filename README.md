...
<p align="center">
  <img alt="Status: v0.2.23 candidate" src="https://img.shields.io/badge/Status-v0.2.23%20candidate-yellow" />
...
Version `0.2.23` is an unpublished Android AVIF output helper injectable validation seam candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while adding injectable encoder, muxer, output-file, and decode-back validation dependencies to `AndroidAvifOutputHelper` so fake encoded bytes, invalid signatures, decode-back failures, and codec failures can be tested before any production enablement. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.23 candidate.
