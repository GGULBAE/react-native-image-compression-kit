...
<p align="center">
  <img alt="Status: v0.2.41 candidate" src="https://img.shields.io/badge/Status-v0.2.41%20candidate-yellow" />
...
Version `0.2.41` is an unpublished iOS PASS payload schema matrix helper candidate for `react-native-image-compression-kit`. It keeps Android and iOS runtime behavior unchanged, keeps AVIF output disabled, does not force AVIF input availability or unavailability, does not force WebP output availability, and refactors the simulator-free `RNICK_IOS_SMOKE_PASS` schema helpers so WebP-output available/unavailable and AVIF-input available/unavailable required field sets are generated from one `IOS_SMOKE_PASS_PAYLOAD_SCHEMA_MATRIX` instead of duplicated arrays. `test/iosSmokeContract.test.mjs` now builds prefixed PASS log fixtures through a fixture factory for all four WebP output x AVIF input combinations, pins payload key order and type schema for platform, JPEG/PNG/GIF/WebP/HEIC/HEIF result byte fields, conditional AVIF result byte fields, PNG-output result byte fields, conditional WebP output byte fields, `webpTargetSizeResultBytes`, `targetSizeResultBytes`, `webpOutputAvailable`, `avifInputAvailable`, `unsupportedInputs`, and `unsupportedOutputs`, keeps missing conditional WebP and AVIF payload field coverage, proves `unsupportedInputs` includes `avif` when AVIF source support is unavailable, and proves `unsupportedOutputs` excludes `webp` when WebP destination support is available. The workflow still tees failed smoke output to `ios-smoke-diagnostics/ios-smoke.log`, generates `ios-smoke-diagnostics/ios-smoke-summary.md`, and uploads the `ios-smoke-diagnostics` artifact only through `if: failure()` steps. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.40`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.40` remains the latest published npm package and iOS AVIF-input unavailable PASS payload schema snapshot release. No npm publish, git tag, or GitHub Release is part of the v0.2.41 candidate.
...
```bash
pnpm smoke:registry -- --version 0.2.38
```
