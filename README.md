...
<p align="center">
  <img alt="Status: v0.2.32 candidate" src="https://img.shields.io/badge/Status-v0.2.32%20candidate-yellow" />
...
Version `0.2.32` is an unpublished iOS smoke timeout CLI fixture coverage candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while moving the CLI timeout diagnostic assembly behind `createSmokeTimeoutErrorFromCLIState()` and the retry warning order behind `formatSmokeRetryWarningMessages()`. `test/iosSmokeCliTimeout.test.mjs` now fixes simulator-free fixture expectations for fake launch output, captured `RNICK_IOS_SMOKE_*` log stream output, Metro output, unified log output, app container and process lookups, and diagnostics-before-retry warning order. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.32 candidate.
...
The retry and timeout diagnostic contract is also covered without Xcode, Metro, or a simulator by `test/iosSmokeContract.test.mjs` and `test/iosSmokeCliTimeout.test.mjs`. The Node-level Vitest suites validate `scripts/ios-smoke-contract.mjs` defaults and overrides for `RNICK_IOS_SMOKE_ATTEMPTS`, `RNICK_IOS_SMOKE_LOG_STREAM_WARMUP_MS`, and `RNICK_IOS_SMOKE_DIAGNOSTIC_LOG_WINDOW`, timeout-only retry decisions, diagnostic formatting, CLI timeout input assembly from fake launch/log stream/Metro/unified-log output, and diagnostics-before-retry warning order.
