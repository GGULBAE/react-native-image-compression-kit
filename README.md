...
<p align="center">
  <img alt="Status: v0.2.29 candidate" src="https://img.shields.io/badge/Status-v0.2.29%20candidate-yellow" />
...
Version `0.2.29` is an unpublished Android AVIF output helper validation-result provenance contract candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while pinning which validation file produced helper `details` for `AndroidAvifOutputHelper`. Direct validation details now retain the direct file name, byte size, signature result, decode-back result, and decoded dimensions when available. Direct failure followed by muxed success or failure now preserves encoder -> direct validation -> muxer -> final validation detail order, with final validation details retaining the muxed file name, byte size, signature result, and decode-back result. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.29 candidate.
