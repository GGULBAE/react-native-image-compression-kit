...
<p align="center">
  <img alt="Status: v0.2.30 candidate" src="https://img.shields.io/badge/Status-v0.2.30%20candidate-yellow" />
...
Version `0.2.30` is an unpublished iOS smoke retry and diagnostic hardening candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while making `scripts/ios-validation.mjs smoke` more resilient when the app launches but `RNICK_IOS_SMOKE_PASS` is not observed before timeout. The iOS smoke runner now supports `RNICK_IOS_SMOKE_ATTEMPTS`, warms the `RNICK_IOS_SMOKE_*` log stream before each app launch, retries timeout-only smoke attempts with a fresh app launch, and prints simulator state, app container paths, app process lookup, launch output, captured smoke stream tail, Metro output tail, and recent unified logs from `RNICK_IOS_SMOKE_DIAGNOSTIC_LOG_WINDOW` before retrying or failing. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.30 candidate.
