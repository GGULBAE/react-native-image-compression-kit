...
<p align="center">
  <img alt="Status: v0.2.27 candidate" src="https://img.shields.io/badge/Status-v0.2.27%20candidate-yellow" />
...
Version `0.2.27` is an unpublished Android AVIF output helper blocked-route detail contract candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while pinning `sdk_unavailable` and `no_image_avif_encoder` helper `details` order and the smoke adapter preservation contract. Blocked helper and smoke results now have tests for route blockers first, then `INJECTABLE_VALIDATION_SEAM`, then `HELPER_DISABLED_FROM_COMPRESS_IMAGE`; `runEncodeDecodeBackSmoke()` also preserves the helper `blockerCode` and `details` while keeping `outputCanBeEnabled=false`. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.27 candidate.
