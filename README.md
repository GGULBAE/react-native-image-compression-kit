...
<p align="center">
  <img alt="Status: v0.2.31 candidate" src="https://img.shields.io/badge/Status-v0.2.31%20candidate-yellow" />
...
Version `0.2.31` is an unpublished iOS smoke diagnostic testability hardening candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while extracting the iOS smoke retry, environment override, and timeout diagnostic formatting contract into `scripts/ios-smoke-contract.mjs`. `test/iosSmokeContract.test.mjs` now fixes the simulator-free Node-level expectations for `RNICK_IOS_SMOKE_ATTEMPTS`, `RNICK_IOS_SMOKE_LOG_STREAM_WARMUP_MS`, `RNICK_IOS_SMOKE_DIAGNOSTIC_LOG_WINDOW`, timeout-only retry decisions, and diagnostic blocks that include simulator, app container, process lookup, launch output, captured `RNICK_IOS_SMOKE_*` stream, Metro output, and unified log state. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.31 candidate.
...
The retry and timeout diagnostic contract is also covered without Xcode, Metro, or a simulator by `test/iosSmokeContract.test.mjs`. That Node-level Vitest suite validates `scripts/ios-smoke-contract.mjs` defaults and overrides for `RNICK_IOS_SMOKE_ATTEMPTS`, `RNICK_IOS_SMOKE_LOG_STREAM_WARMUP_MS`, and `RNICK_IOS_SMOKE_DIAGNOSTIC_LOG_WINDOW`, timeout-only retry decisions, and diagnostic blocks with simulator/app/process/launch/RNICK stream/Metro/unified log fields.
