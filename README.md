...
<p align="center">
  <img alt="Status: v0.2.48 npm latest" src="https://img.shields.io/badge/Status-v0.2.48%20npm%20latest-brightgreen" />
...
Run `pnpm smoke:registry -- --version 0.2.48 --expect-tag latest --json --report-file registry-provenance.json` to reproduce the published registry evidence. `--json` writes exactly one canonical object to stdout, and `--report-file` atomically writes the same bytes with fixed fields from `schemaVersion` through `error`. The successful manual [Registry Validation run](https://github.com/GGULBAE/react-native-image-compression-kit/actions/runs/29181708376) appends the evidence to the GitHub Step Summary and uploads matching provenance report/stdout artifacts.
...
For the published v0.2.48 npm release, run the registry-based provenance smoke:
