...
<p align="center">
  <img alt="Status: v0.2.49 candidate" src="https://img.shields.io/badge/Status-v0.2.49%20candidate-orange" />
...
Version `0.2.49` is an unpublished Registry provenance bundle offline verification candidate for `react-native-image-compression-kit`; npm `latest` remains the published `0.2.48` release. It keeps native behavior, the public API, and the registry report schema unchanged. The candidate atomically bundles the exact tarball consumed by registry smoke with canonical report/stdout files and a checksum manifest, then verifies the bundle without npm, GitHub, or other network access.
