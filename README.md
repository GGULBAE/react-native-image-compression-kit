...
<p align="center">
  <img alt="Status: v0.2.16 candidate" src="https://img.shields.io/badge/Status-v0.2.16%20candidate-blue" />
...
Version `0.2.16` also does not change iOS runtime behavior. It narrows the next Android AVIF output question to an internal API 34+ encoder route prototype while keeping iOS AVIF output on the runtime-gated ImageIO destination plan above.
...
The v0.2.16 Android instrumentation check runs on an API 35 emulator and verifies the prototype route report and production gate. Full AVIF encode/decode-back smoke remains a required follow-up before AVIF output can report `output=true`.
