...
<p align="center">
  <img alt="Status: v0.2.28 candidate" src="https://img.shields.io/badge/Status-v0.2.28%20candidate-yellow" />
...
Version `0.2.28` is an unpublished Android AVIF output helper temp-file lifecycle contract candidate for `react-native-image-compression-kit`. It keeps Android and iOS AVIF output disabled while pinning direct and muxed validation temp-file behavior for `AndroidAvifOutputHelper`. Direct validation success now has tests proving only the direct file is created, no muxed file is created, and `outputFilePath`/`byteSize` come from the direct file. Direct failure followed by muxed success now has tests proving `outputFilePath`/`byteSize` come from the muxed file and the direct intermediate file is not reported as the result. Invalid-signature and decode-back failure tests now pin the final blocker file path and `byteSize` to the final muxed validation file. The Android `compressImage()` scaffold still rejects `output.format: 'avif'` with `ERR_NOT_IMPLEMENTED` before source access or helper entry, keeps `avif.output=false`, and keeps metadata `preserve`, `output.maxBytes`, and animated AVIF boundaries explicit. npm `latest` remains `0.2.19`; the previous GitHub Release remains [v0.2.17](https://github.com/GGULBAE/react-native-image-compression-kit/releases/tag/v0.2.17).

Version `0.2.19` remains the latest published npm package and AVIF output production gate release. No npm publish, git tag, or GitHub Release is part of the v0.2.28 candidate.
