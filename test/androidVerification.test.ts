...
describe('Android verification scripts', () => {
  it('declares the v0.2.62 documentation candidate metadata', () => {
    ...
    const staleReadmeSnippets = [
      ...
      'Status: v0.2.53 candidate',
      'v0.2.53%20candidate',
      'The repository package metadata is `0.2.53` for the unpublished GitHub Action pin update provenance and manual review gate candidate.',
    ...
    expect(readmeSource).toContain(
      'Version `0.2.60` was the previous unpublished release evidence review archive import and expiration-independent replay gate candidate.'
    );
    expect(readmeSource).toContain(
      'Version `0.2.59` was the previous unpublished release evidence policy review receipt and manual promotion rehearsal candidate.'
    );
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain(
      'Version `0.2.56` was the previous unpublished release evidence archive import automation and multi-version regression gate candidate.'
    );
    expect(readmeSource).toContain(
      'Version `0.2.55` was the previous npm `latest` Action Pin artifact GitHub OIDC attestation and offline signer verification release.'
    );
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain(
      'Version `0.2.52` was the previous unpublished immutable GitHub Actions pin and workflow supply-chain gate candidate.'
    );
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain(
      'Version `0.2.50` was the previous npm `latest` GitHub artifact attestation and offline identity verification release for `react-native-image-compression-kit`.'
    );
    expect(readmeSource).toContain(
      'Version `0.2.49` was the previous unpublished Registry provenance bundle offline verification candidate.'
    );
    expect(readmeSource).toContain(
      'Version `0.2.48` was the previous npm `latest` registry provenance and manual CI gate release for `react-native-image-compression-kit`.'
    );
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain('pnpm acquire:release-evidence-review --');
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain('pnpm import:release-evidence-review --');
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain(
      'aggregate archive SHA-256 `f63924d58ef18c94379b102949e6870e838a014ac883b7c9c03fca5abc6b56dd`'
    );
    expect(readmeSource).toContain(
    ...
    expect(readmeSource).toContain('pnpm import:release-evidence --');
    expect(readmeSource).toContain(
      'pnpm verify:release-evidence-set -- --json'
    );
    expect(readmeSource).toContain(
      'With no selectors the command verifies v0.2.50 and v0.2.55'
    );
    expect(readmeSource).toContain(
  ...
  it('wires the post-publish registry package smoke test', () => {
    ...
    expect(registryWorkflowSource).toContain('default: "0.2.55"');
  ...
  it('documents the v0.2.62 documentation candidate and retained v0.2.55 evidence', () => {
    ...
    expect(releaseSource).toContain('### Explicit Acquisition Contract');
    expect(releaseSource).toContain('### Canonical Acquisition and Handoff');
    expect(releaseSource).toContain(
      'pnpm fixtures:release-evidence-review-acquisition:check'
    );
    expect(releaseSource).toContain('## v0.2.60');
    expect(releaseSource).toContain(
    ...
    expect(releaseSource).toContain('### Canonical Archive Contract');
    expect(releaseSource).toContain('### Retained v0.2.55 Review Evidence');
    expect(releaseSource).toContain('pnpm import:release-evidence-review --');
    expect(releaseSource).toContain(
      'pnpm verify:release-evidence-review-archive-set -- --json'
    );
    expect(releaseSource).toContain(
    ...
    expect(releaseSource).toContain('### Canonical Candidate and Diff');
    expect(releaseSource).toContain('### Reviewed Promotion Gate');
    expect(releaseSource).toContain(
      '`pnpm prepare:release-evidence-policy -- --acquisition-dir <path> --candidate-file <candidate.json> --json --report-file <report.json>`'
    );
    expect(releaseSource).toContain('### Live v0.2.55 Replay Evidence');
    expect(releaseSource).toContain(
      'aade4a8057bbb8f6b3dc92690b3d9cc5e3b57352a5734396e3921a143a449f8d'
    );
    expect(releaseSource).toContain('## v0.2.57');
    expect(releaseSource).toContain(
    ...
    expect(releaseSource).toContain('### Offline Archive Import Contract');
    expect(releaseSource).toContain('### Multi-version Regression Contract');
    expect(releaseSource).toContain(
    ...
    expect(releaseSource).toContain('## v0.2.53');
    expect(releaseSource).toContain(
    ...
    expect(releaseSource).toContain('### Action Pin Provenance Contract');
    expect(releaseSource).toContain(
  ...
  it('documents security policy and package hygiene expectations', () => {
    ...
    expect(securitySource).toContain('## Release Evidence Retention');
    expect(securitySource).toContain('pnpm acquire:release-evidence --');
    expect(securitySource).toContain('infer or select the latest run');
    expect(securitySource).toContain('download each ZIP by artifact ID');
    expect(securitySource).toContain(
    ...
    expect(securitySource).toContain('pnpm acquire:release-evidence-review --');
    expect(securitySource).toContain(
      'pnpm fixtures:release-evidence-review-acquisition:check'
    );
    expect(securitySource).toContain('pnpm prepare:release-evidence-policy --');
    expect(securitySource).toContain(
      'Policy source changes require a normal reviewed Git commit.'
    );
    expect(securitySource).toContain('pnpm promote:release-evidence-policy --');
    expect(securitySource).toContain(
      'Preserve an approved review after GitHub artifact expiration'
    );
    expect(securitySource).toContain('pnpm import:release-evidence-review --');
    expect(securitySource).toContain(
    ...
    expect(securitySource).toContain('pnpm import:release-evidence --');
    expect(securitySource).toContain(
      'The metadata must exactly match the committed immutable version policy.'
    );
    expect(securitySource).toContain(
      'fully verifies a sibling temporary archive before one atomic rename'
    );
    expect(securitySource).toContain(
      'pnpm verify:release-evidence-set -- --json'
    );
    expect(securitySource).toContain(
      'The default ordered set contains v0.2.50 and v0.2.55'
    );
